...
export function TrapezoidNode({ data, selected }: NodeProps) {
  const color = selected ? "#ff9500" : "#f59e0b";
  return (
    <div
      className={`mve-node mve-trapezoid ${selected ? "mve-selected" : ""}`}
      style={{
    ...
    >
      ...
      <svg className="mve-shape-bg" viewBox="0 0 160 48" preserveAspectRatio="none">
        <defs>
          <filter id="trapezoid-shadow" x="-4%" y="-4%" width="108%" height="116%">
            <feDropShadow dx="0" dy="1" stdDeviation="1.5" floodOpacity="0.08" />
          </filter>
        </defs>
        <polygon
          points="20,2 140,2 158,46 2,46"
          fill="var(--mve-bg, #fff)" stroke={color} strokeWidth="1.5"
          strokeLinejoin="round" filter="url(#trapezoid-shadow)"
        />
      </svg>
